<script>
    ...
    const lang = {
        en: {
            title: 'Your playlists - BassieMusic',
        ...
        nl: {
            title: 'Jouw afspeellijsten - BassieMusic',
            header: 'Jouw afspeellijsten',
    ...
    lazyLoader(
        ...
        async (page) => {
            const response = await fetch(
                `${import.meta.env.VITE_API_URL}/users/${data.authUser.id}/playlists?${new URLSearchParams({
                    page,
                    sort_by: data.sortBy,
                })}`,
                {
