<script>
import { PLAYER_UPDATE_UI_TIMEOUT, PLAYER_UPDATE_SERVER_TIMEOUT, PLAYER_SEEK_TIME } from '../config.js';
import { playingTrack, playingQueue, audioVolume } from '../stores.js';
import { formatDuration } from '../filters.js';

let isPlaying = false, audio, audioDuration, audioCurrentTime;
let oldAudioVolume;
...
async function updateServerLoop() {
    await fetch(`${import.meta.env.VITE_API_URL}/tracks/${track.id}/play`, {
        headers: {
...
function previousTrack() {
    playingTrack.update(playingTrack => playingTrack - 1 >= 0 ? playingTrack - 1 : $playingQueue.length - 1);
}
...
function nextTrack() {
    playingTrack.update(playingTrack => playingTrack + 1 <= $playingQueue.length - 1 ? playingTrack + 1 : 0);
}
    ...
    {#if $playingQueue.length > 0}
        <div style="display: flex; align-items: center;">
            ...
            <div style="flex: 4; display: flex;">
                ...
                <input type="range" style="flex: 1;" value="{audioCurrentTime}" on:input={seekToInput} min="0" max="{audioDuration}">
